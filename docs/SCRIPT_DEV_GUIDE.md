...
本文档介绍如何为 Mira 编写、构建和维护自动化脚本。这些脚本由 Mira 根据用户指令调用，可扩展设备控制、UI 自动化、网络请求和文件操作等能力。Mira 继续兼容 Operit 脚本协议。
...
*   **路径 A：在 Mira 项目中直接开发 (推荐)**: 这是最简单快捷的方式，无需手动配置环境。适合想要快速编写脚本并贡献给主项目的开发者。
*   **路径 B：创建独立的脚本项目 (高级)**: 如果你想创建和维护自己的脚本仓库，或者将脚本作为独立产品发布，可以选择此路径。这需要你手动完成项目的搭建。

---

### 路径 A: 在 Mira 项目中直接开发 (5分钟快速入门)

这是最推荐的入门方式。Mira 仓库已经包含脚本示例、类型定义、调试工具和打包流程。
...
每个脚本文件的开头都必须包含一个 `/* METADATA ... */` 注释块。这个块定义脚本名称、描述、分类以及所提供的工具。Mira 会解析 `METADATA`，再把这些能力作为可用工具提供给大语言模型。
