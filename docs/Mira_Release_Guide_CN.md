...
`.properties` 的值不要额外包双引号，否则引号会成为路径或密码的一部分。`local.properties`、keystore 和密码不要提交到 Git。所有公开版本必须使用同一份 keystore，否则 Android 不接受覆盖安装。keystore 至少保留两份离线备份。

GitHub 登录使用 OAuth Device Flow。正式构建必须配置已开启 Device Flow 的公开 `GITHUB_CLIENT_ID`；APK 不需要也不应携带 OAuth Client Secret。用户在系统浏览器输入一次性 user code 完成授权，应用只与 `github.com` 和 `api.github.com` 交换 Device Code、Token 与 GitHub 数据。

项目同步、Debug 构建和 Debug 单测不要求发布签名；执行 Release 构建时会先运行 `validateReleaseSigning`，签名字段不完整或 keystore 路径不存在会明确失败。
...
- GitHub 设置页能够生成验证地址和一次性 user code，系统浏览器授权后回到应用自动完成登录。
- 登出再换另一个 GitHub 账号，用户资料、Token 和市场管理身份随账号切换，不复用上一账号会话。
- 首次发布在当前账号创建或使用 `MiraForge`，并在 `kernelx30/Mira` 创建登记 Issue，市场条目标识为 `mira-<issueNumber>`。
- 评论和点赞分别落到 GitHub Comment 与 Reaction；删除登录状态后不再允许写入。
- Operit 兼容条目通过 Mira Pages 镜像只读展示，仍从原仓库或 Release 下载，不向旧市场服务发送 Mira GitHub Token。
...
- `GITHUB_CLIENT_ID` 已配置且对应的 OAuth App 已启用 Device Flow；APK 中不存在 Client Secret。
- GitHub 登录、退出、换号、MiraForge 发布、Issue 登记、评论与 Reaction 已完成真机验收。
- 网络检查确认 GitHub Token 只发送到 `github.com` 和 `api.github.com`，旧 Operit 市场仅走只读 Pages 数据。
