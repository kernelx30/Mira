...
| 应用控制 | 主要通过设置页面逐项配置 | 增加对话内设置工具，可搜索、读取和修改会话、角色及全局设置 |
| 插件生态 | Operit 市场、Skill、MCP、ToolPkg | Mira 静态市场入口与 MiraForge 新发布，同时兼容既有 OperitForge 资源 |
| 数据迁移 | 聊天、配置与数据库备份基础 | 增加统一记忆包、原始数据快照、定时 Room 备份和 Operit 聊天备份识别导入 |
...
- 供应商按实际能力分别使用原生情绪字段、风格指令、韵律参数或普通朗读
...
- 直接说“打开自动朗读”“关闭沉浸对话”或“把表现力度设为鲜明”，由 `manage_mira_settings` 搜索、读取并修改真实设置
- 会话覆盖、当前角色偏好和全局默认分别保存，工具结果会返回实际作用域与修改后的值
- API Key、清空重置、导入覆盖、Android 权限和悬浮窗服务不交给通用设置工具处理
- 扩展管理器统一承载脚本、ToolPkg、Skill 与 MCP；ToolPkg 的容器和子包分别启用，角色再按需授权能力
- Mira 市场负责公开元数据与来源追踪，新资源发布到用户自己的 `MiraForge`，并继续识别 Operit 兼容资源
- 备份页面提供聊天、角色卡、模型配置、结构化记忆、Room 数据库和原始数据快照；也可识别并导入支持的 Operit 聊天备份
...
Mira 使用 GitHub OAuth Device Flow 登录：应用展示一次性 user code，用户在系统浏览器打开 GitHub 验证页完成授权。公开 APK 只包含公开 Client ID，不包含 Client Secret，也不使用回调 WebView。取得的 Token 只发送到 `github.com` 与 `api.github.com`。

新扩展先发布到用户自己的 `MiraForge`，再通过 `kernelx30/Mira` 的 GitHub Issue 登记市场元数据，条目标识为 `mira-<issueNumber>`；评论、点赞和管理分别使用 GitHub Issues、Comments 与 Reactions。已有 Operit 市场条目通过 Mira Pages 镜像只读展示，并继续从作者原仓库或 Release 下载，不再把 Mira GitHub Token 交给旧市场服务。
...
| GitHub 登录与市场互动 | Device Flow 登录、MiraForge 发布、Issue 登记、评论与 Reaction 只访问 GitHub 官方域名 |

安全要点：

- Mira 不使用自建聊天中转服务器；云端请求直达用户配置的模型或语音 Endpoint
- GitHub 登录不需要把 Client Secret 放进 APK；授权 Token 只用于 GitHub 官方 API，不发送到 Operit 市场服务
...
参考 `local.properties.example` 配置本机 Android SDK、GitHub OAuth 公开 Client ID 和 release 签名字段。真实 API Key、keystore 与密码留在本机，不提交到 Git；Device Flow 不需要 OAuth Client Secret。
